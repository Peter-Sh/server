...
bool has_proxy_protocol_header(NET *net)
{
  ...
  return (memcmp(preread_bytes, PROXY_PROTOCOL_V1_SIGNATURE, NET_HEADER_SIZE) == 0 ||
      memcmp(preread_bytes, PROXY_PROTOCOL_V2_SIGNATURE, NET_HEADER_SIZE) == 0);
}
...
bool is_proxy_protocol_allowed(const sockaddr *addr)
{
  ...
  */
  ...
  {
    int len= (addr->sa_family == AF_INET) ? sizeof(sockaddr_in): sizeof (sockaddr_in6);
    int dst_len;
